...
class Mail
{
	...
	public static function sendMessage(array $d)
	{
		...
		$to = isset($app["contactMail"]["email"]) ? $app["contactMail"]["email"] : false;
		...
		if (self::send($to, $d["email"], $d["email"], "Message from " . $app["siteName"], $r) === true) {
			...
			return Alert::render("Ooops! Something went wrong! Try sending your message again.", "alert error");
		}
	}
	...
	public static function send($t, $f, $fn, $s, $b)
	{
		...
		if (isset($app["SMTP"]["host"]) && isset($app["SMTP"]["port"]) && isset($app["SMTP"]["login"]) && isset($app["SMTP"]["password"])) {
			$m->IsSMTP();
			$m->Host = $app["SMTP"]["host"];
			$m->SMTPAuth = true;
			$m->Port = $app["SMTP"]["port"];
			$m->Username = $app["SMTP"]["login"];
			$m->Password = $app["SMTP"]["password"];
			//$m->SMTPSecure = "ssl";
		...
		if (!$m->Send()) {
			Debug::err(new appException($m->ErrorInfo));
			return false;
		}
...
class PHPMailer
{
	...
	{
		if ($advanced) {
			require_once 'extras/html2text.php';
